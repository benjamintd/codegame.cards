export default {
  ...
  "rules-classic-grid": `25 zufällige Wortkarten werden in einem 5-mal-5-Raster ausgelegt. Ein Teil dieser Wörter steht für <1>rote</1> Agent:innen, ein anderer Teil für <3>blaue</3> Agent:innen, eines für einen <5>Attentäter</5>, und die anderen repräsentieren <7>unbeteiligte Zuschauer:innen</span>. Die Geheimdienstchef:innen der Teams kennen die Farben der einzelnen Wörter und geben ihrem Team abwechselnd Hinweise, damit sie die Wörter ihrer Farbe erraten.`,
  ...
  "rules-duet-hints": `Die Spieler:innen geben abwechselnd Hinweise, damit der/die andere Spieler:in eine grüne Karte errät. Jeder Hinweis darf nur aus einem einzigen Wort und einer Anzahl von Karten bestehen, auf die er sich bezieht. Der/die Geheimdienstchef:in gibt einen Hinweis, der sich auf möglichst viele der Wörter auf den Karten der eigenen Agent:innen bezieht, aber nicht auf andere - sie könnten den anderen Spieler oder die andere Spielerin versehentlich dazu verleiten, eine Karte zu wählen, die eine:n unschuldige:n Zuschauer:in oder einen Attentäter darstellt`,
